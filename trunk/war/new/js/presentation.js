...
$(document).ready(
		function() {
			$.getScript('js/number-commas-sort.js'); // Show commas thousands
														// in numbers.
			...
			tableDef.aoColumns = [ {
				...
				sTitle : "הקצאה נטו",
				sType : "number-commas",
				fnRender : renderNumberWithCommas
			}, {
			...
			for ( var i = MIN_YEAR; i <= thisYear; ++i) {
				select.append($("<option></option>").attr("value", i).attr(
						"selected", "selected").text(i));
			}
...
function prepareUserBuckets() {
	...
	$('#saveBucketButton').click(function() {
		saveBucket();
	});
...
function refreshUI() {
		...
		for (year in expense.years) {
			...
			if (!sums[code]) {
				...
				if (sums[code].title != item.title
						&& $.inArray(code, expensesWarned) == -1) {
					expensesWarned.push(code);
					prettyAlert('שימו לב: סעיף '
							+ code
							+ ' מכיל לאורך השנים תיאורים שונים.</br>'
			...
			row.push('<a href="javascript:deleteExpense(\'' + item.code
					+ '\')">מחקו</a>');
	...
	$.each(sums, function(i, item) {
		...
		groupTr.append($("<td>").addClass(tdClass).text(
				parseInt(item.net_allocated * item.weight)));
		row.push(parseInt(item.net_revised * item.weight));
		groupTr.append($("<td>").addClass(tdClass).text(
				parseInt(item.net_revised * item.weight)));
		row.push(parseInt(item.gross_revised * item.weight));
		groupTr.append($("<td>").addClass(tdClass).text(
				parseInt(item.gross_revised * item.weight)));
		row.push(parseInt(item.net_used * item.weight));
		groupTr.append($("<td>").addClass(tdClass).text(
				parseInt(item.net_used * item.weight)));
		row.push(parseInt(item.gross_used * item.weight));
		groupTr.append($("<td>").addClass(tdClass).text(
				parseInt(item.gross_used * item.weight)));
		row.push('<a href="javascript:deleteExpense(\'' + item.code
				+ '\')">מחקו</a>');
		groupTr.append($("<td>").addClass(tdClass).text(""));
		// oTable.fnAddData(row);
