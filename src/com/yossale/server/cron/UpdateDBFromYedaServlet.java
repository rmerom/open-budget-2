...
public class UpdateDBFromYedaServlet extends HttpServlet {
	...
	private String fetchYearSections(int requestedYear) throws IOException, JSONException {
  	URL url = new URL("http://api.yeda.us/data/gov/mof/budget/" 
  			+ "?o=json&query=" + URLEncoder.encode("{\"year\":"   
	      + requestedYear + "}", "utf-8") + "&limit=" + MAX_SECTIONS_PER_YEAR);
  	logger.warning("fetching: " + requestedYear + " from url: " + url.toString());
